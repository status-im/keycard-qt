...
private:
    ...
    QByteArray validSelectResponse() const {
        const QByteArray publicKey = secp256k1Generator();
        QByteArray body;
    ...
    QByteArray preInitializedSelectResponse() const {
        const QByteArray publicKey = secp256k1Generator();
        QByteArray response;
        response.append(static_cast<char>(0x80));
        response.append(static_cast<char>(0x41));
        response.append(publicKey);
        response.append(QByteArray::fromHex("9000"));
        return response;
    }

    QByteArray secp256k1Generator() const {
...
private slots:
    ...
    void testFailedOrInterruptedSelectClosesSecureChannel() {
        m_mock->queueResponse(validSelectResponse());
        QVERIFY(m_cmdSet->select().initialized);
        ...
        m_mock->queueResponse(QByteArray::fromHex("6a82"));
        QVERIFY(!m_cmdSet->select(true).installed);
        QVERIFY(!m_cmdSet->testSecureChannelIsOpen());

        m_cmdSet->testInjectSecureChannelState(
            pairing, QByteArray(16, 0x55), QByteArray(32, 0x66), QByteArray(32, 0x77));
        QVERIFY(m_cmdSet->testSecureChannelIsOpen());
        m_mock->setNextTransmitThrows(QStringLiteral("SELECT interrupted"));
        QVERIFY_THROWS_EXCEPTION(std::runtime_error, m_cmdSet->select(true));
        QVERIFY(!m_cmdSet->testSecureChannelIsOpen());
    }

    void testMutualAuthenticationExceptionClosesSecureChannel() {
        m_mock->queueResponse(validSelectResponse());
        QVERIFY(m_cmdSet->select().initialized);

        int transmissions = 0;
        m_mock->setResponseHandler([this, &transmissions](const QByteArray&) {
            if (transmissions++ == 0) {
                return openSecureChannelResponse();
            }
            throw std::runtime_error("Mutual authentication transport failure");
        });

        const PairingInfo pairing(QByteArray(32, 0x44), 0);
        QVERIFY_THROWS_EXCEPTION(
            std::runtime_error, m_cmdSet->openSecureChannel(pairing));
        QVERIFY(!m_cmdSet->testSecureChannelIsOpen());
    }

    void testInitFailsWhenCardDoesNotReportInitialized() {
        ...
        QVERIFY(m_cmdSet->lastError().contains("did not report initialized"));
        QVERIFY(!m_cmdSet->testSecureChannelIsOpen());
    }
